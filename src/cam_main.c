...
int main() {
  ...
  char * vertexShaderSource = get_shader_content("../src/shaders/tri_vertex.glsl");
  ...
  char * fragmentShaderSource = get_shader_content("../src/shaders/tri_fragment.glsl");
  ...
  // configuring how vertices should be handled
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
  glEnableVertexAttribArray(0);

  // configuring the vertices color information
  // key change is here: make the GLuint index match those in shader program
  ...
  while(!glfwWindowShouldClose(window)) {
    ...
    // utilizing glfwGetTime for the pulsing color
